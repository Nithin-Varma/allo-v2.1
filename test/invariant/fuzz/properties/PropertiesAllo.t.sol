...
contract PropertiesAllo is HandlersParent {
    ...
    function prop_onlyProfileOwnerCanAddProfileMember(
        ...
        if (msg.sender == _profile.owner) {
            if (_success) {
                assertTrue(
                    registry.hasRole(_profile.id, _newMember),
                    "property-id 5-a: addMembers failed role not set"
                );
                _ghost_roleMembers[_profile.id].push(_newMember);
            } else {
                assertTrue(
                    _newMember == address(0),
                    "property-id 5-a: addMembers failed"
                );
            }
        } else {
            if (_success) {
                fail(
                    "property-id 5-a: addMembers only owner should be able to add members"
                );
            }
        }
    }
    ...
    function prop_onlyProfileOwnerCanRemoveProfileMembers() public {
        ...
        vm.prank(msg.sender);
        (bool _success, ) = address(registry).call(
            abi.encodeCall(registry.removeMembers, (_profile.id, _members))
        );

        if (msg.sender == _profile.owner) {
            if (_success) {
                assertTrue(
                    !registry.hasRole(_profile.id, _memberToRemove),
                    "property-id 5-b: removeMembers failed"
                );
                _ghost_roleMembers[_profile.id].pop();
            } else {
                assertTrue(
                    _memberToRemove == address(0),
                    "property-id 5-b: removeMembers failed"
                );
            }
        } else {
            if (_success) {
                fail(
                    "property-id 5-b: removeMembers only owner should be able to remove members"
                );
            }
        }
    }
        ...
        if (msg.sender == _profile.owner) {
            if (_success) {
                assertTrue(
                    ...
                    "property-id 6: updateProfilePendingOwner success but wrong pending owner"
                );
            } else {
                assertTrue(
                    _newOwner == address(0),
                    "property-id 6: updateProfilePendingOwner failed with non-zero address and correct caller"
                );
            }
        } else {
            if (_success) {
                fail(
                    "property-id 6: updateProfilePendingOwner success but not owner caller"
                );
            }
        }
    }
    ...
    function prop_poolAdminCanAlwaysChangeAdminToNonZero(
        uint256 _idSeed,
        uint256 _actorSeed
    ) public {
        _idSeed = bound(_idSeed, 0, ghost_poolIds.length - 1);
        ...
        if (_success) {
            ...
            assertTrue(
                allo.hasRole(_poolAdminRole, _newAdmin),
                "property-id 10: changeAdmin failed role not set"
            );
            assertTrue(
                allo.isPoolAdmin(_poolId, _newAdmin),
                "property-id 10: admin not set"
            );
            ghost_poolAdmins[_poolId] = _newAdmin;
        } else {
            assertTrue(
                _newAdmin == address(0) || msg.sender != _admin,
                "property-id 10: changeAdmin failed"
            );
        }
    }
    ...
    function prop_poolAdminCanAlwaysAddManagers(
        uint256 _idSeed,
        address _newManager
    ) public {
        _idSeed = bound(_idSeed, 0, ghost_poolIds.length - 1);
        ...
        if (_success) {
            assertEq(
                msg.sender,
                _admin,
                "property-id 11-a: addPoolManagers only admin should be able to add managers"
            );
            assertTrue(
                allo.hasRole(_poolManagerRole, _newManager),
                "property-id 11-a: addPoolManagers failed role not set"
            );
            ghost_poolManagers[_poolId].push(_newManager);
        } else {
            assertTrue(
                _newManager == address(0) || msg.sender != _admin,
    ...
    function prop_poolAdminCanAlwaysRemoveManagers(
        uint256 _idSeed,
        ...
        _managerIndex = bound(
            _managerIndex,
            0,
            ghost_poolManagers[_poolId].length - 1
        );
        address _admin = ghost_poolAdmins[_poolId];
        ...
        vm.prank(msg.sender);
        (bool _success, ) = address(allo).call(
            abi.encodeCall(allo.removePoolManagers, (_poolId, _removeManagers))
        );
        ...
        if (_success) {
            assertEq(
                msg.sender,
                _admin,
                "property-id 11-b: removePoolManagers only admin should be able to remove managers"
            );
            assertTrue(
                !allo.hasRole(_poolManagerRole, _manager),
                "property-id 11-b: removePoolManagers failed"
            );
            ...
            assertTrue(
                _manager == address(0) || msg.sender != _admin,
    ...
    function prop_poolManagerCanAlwaysChangeMetadata(
        ...
        if (_success) {
            ...
            assertEq(
                _pool.metadata.protocol,
                _metadata.protocol,
                "property-id 13: updatePoolMetadata protocol failed"
            );
            assertEq(
                _pool.metadata.pointer,
                _metadata.pointer,
                "property-id 13: updatePoolMetadata pointer failed"
            );
        } else {
            assertTrue(
                !_authorized,
                "property-id 13: updatePoolMetadata failed while auth"
            );
        }
    }
    ...
    function prop_alloOwnerCanAlwaysChangeBaseFee(uint256 _newBaseFee) public {
        ...
        if (_success) {
            assertEq(
                allo.getBaseFee(),
                _newBaseFee,
                "property-id 14-a: updateBaseFee failed"
            );
            baseFee = _newBaseFee;
        } else {
    ...
    function prop_alloOwnerCanAlwaysChangePercentFee(
        uint256 _newPercentFee
    ) public {
        vm.prank(allo.owner());
        (bool _success, ) = address(allo).call(
        ...
        if (_success) {
            assertEq(
                allo.getPercentFee(),
                _newPercentFee,
                "property-id 14-b: updatePercentFee failed"
            );
    ...
    function prop_alloOwnerCanAlwaysChangeTreasury(
    ...
    function prop_alloOwnerCanAlwaysChangeTrustedForwarder(
        ...
        if (_success) {
            assertTrue(
                allo.isTrustedForwarder(_newForwarder),
                "property-id 15-b: updateTrustedForwarder failed"
            );
            forwarder = _newForwarder;
        } else {
            assertTrue(
                _newForwarder == address(0),
                "property-id 15-b: updateTrustedForwarder failed"
            );
        }
    }
    ...
    function prop_alloOwnerCanAlwaysChangeRegistry(
        ...
        if (_success) {
            assertEq(
                ...
                _newRegistry,
                "property-id 15-c: updateRegistry failed"
            );
            ...
            assertEq(
                address(allo.getRegistry()),
                address(registry),
                "property-id 15-c: updateRegistry rollback failed"
            );
        } else {
            assertTrue(
    ...
    function prop_alloOwnerCanAlwaysRecoverFundsFromAlloContract(
        uint256 _idSeed
    ) public {
        _idSeed = bound(_idSeed, 0, ghost_poolIds.length - 1);
        uint256 _poolId = ghost_poolIds[_idSeed];
        ...
        address _recipient = _ghost_actors[
            _idSeed % (_ghost_actors.length - 1)
        ];
        ...
        vm.prank(allo.owner());
        (bool _success, ) = address(allo).call(
            abi.encodeCall(allo.recoverFunds, (address(_token), _recipient))
        );
        ...
        if (_success) {
            if (_token == Transfer.NATIVE) {
                ...
                assertEq(
                    address(allo).balance,
                    0,
                    "property-id 16: recoverFunds failed allo balance should  be zero"
                );
            } else {
                assertEq(
                    token.balanceOf(_recipient),
            ...
            assertTrue(
                _previousBalanceAllo == 0,
                "property-id 16: recoverFunds failed"
            );
        }
    }
